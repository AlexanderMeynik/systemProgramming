 ...
 {                                                /*команды A               */
  ...
  unsigned long ARG1 =
          OBLZ[BAS_IND + CUR_IND + sm ] * 0x100 +
           OBLZ[BAS_IND + CUR_IND + sm + 1];//low bytes
  if(VR[R1]==ARG1)
  {
      VR[16] =0;
  }
...
int P_BC(void)
{
    ...
    if (R1 == 15)
    {
        I = ARG;//todo как выичслить адресс по содер памяти
    } else if(R1==8)
    {
        if(VR[16]==0)
        {
...
{                                                /*команды ST              */
    ...
    for (i=2; i<4; i++)                             /*запись преобразованого  */
        OBLZ[BAS_IND + CUR_IND + sm + i-2] = bytes[i];   /*значения по адресу 2-г  */
    /*операнда                */
    return 0;                                       /*успешное заверш.прогр.  */
}
...
{                                                /*команды A               */
    ...
    ADDR = VR[B] + VR[X] + D;                       /*вычисление абс.адреса и */
    sm = ( int ) ( ADDR -I );                       /*смещения                */
    ARG = OBLZ[BAS_IND + CUR_IND + sm ] * 0x100 +     /*лашениях ЕС ЭВМ         */
...
//поле регистров
  ...
  //todo добавить поле для condition code http://www.simotime.com/asmins01.htm#CH
