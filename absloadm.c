...
unsigned long VR[REG_COUNT],                             /*массив,содерж.знач.рег. */
...
{{'C','H',' ',' ',' '} , '\x49' , 4 , FRX} ,
{{'S','T','H',' ',' '} , '\x40' , 4 , FRX} ,
{{'L','H',' ',' ',' '} , '\x48' , 4 , FRX} , /*машинных                */
{{'A','H',' ',' ',' '} , '\x4A' , 4 , FRX} , /*операций                */
{{'S','H',' ',' ',' '} , '\x4B' , 4 , FRX} ,
    };
//..........................................................................
...
int P_CH(void)
{
    ...
    sm = ( int ) ( ADDR -I );                       /*смещения                */
    ARG =
            OBLZ[BAS_IND + CUR_IND + sm ] *  0x1000000L +//для сравнения нам нужно взять 2 байта из памяти(halfword)
            OBLZ[BAS_IND + CUR_IND + sm + 1]* 0x10000L;//а после сместить их так, чтобы можно было сравнивать с одержимым регистра
    if(VR[R1]==ARG)//при равенстве содержимого регистра и памяти CON должен стать 0
    {
        VR[16] =0;
    }
    else if(VR[R1]<ARG)//если содержимое регистра меньше, то 1
    {
        VR[16] =1;
    } else //иначе 2
    {
...
int P_BC(void)
{
    ...
    int new_I= BAS_ADDR + CUR_IND + sm; //вычислим новй адрес в программе
    int new_IND=CUR_IND + sm;//вычислим индекс новой команды
    if (R1 == 15)//безусловный переход
    {
        I = new_I;
        CUR_IND = new_IND;
    }
    ...
    {
        if(VR[16]==0)//если содержимое CON = 0
        {
            I = new_I;
            CUR_IND = new_IND;
        }
...
int P_STH()
{
    ...
    for (i=0; i<2; i++)                             //т.к. мы записываем в старшгие 2 байта регистра, берём только их.
...
int P_LH()
{
    ...
    sm = (int) ( ADDR - I );
    VR[R1] =
            OBLZ[BAS_IND + CUR_IND + sm  ] *  0x1000000L +//загрузим содержимое памяти в 2 старших байта регистра
...
int P_AH()
{
    ...
    ADDR = VR[B] + VR[X] + D;                       /*вычисление абс.адреса и */
    sm = ( int ) ( ADDR -I );                       /*смещения                */
    ARG = OBLZ[BAS_IND + CUR_IND + sm ] *  0x1000000L +    //для корректного сложения надо получить
          OBLZ[BAS_IND + CUR_IND + sm + 1]* 0x10000L;      //для аргумента аналогичное представление
...
int P_SH()
{
    ...
    ADDR = VR[B] + VR[X] + D;                       /*вычисление рабочего     */
    sm = ( int ) ( ADDR - I );                      /*адреса и смещения       */
    ARG = OBLZ[BAS_IND + CUR_IND + sm ] *  0x1000000L +     //аналогично, нам нужно получить нужное представление
          OBLZ[BAS_IND + CUR_IND + sm + 1]* 0x10000L;
    VR[R1] = VR[R1] - ARG;                          /*выч-ие из 1-го операнда */
    return 0;                                       /*успешное заверш.прогр.  */
}
...
//поле регистров
  wblue = newwin(REG_COUNT, 12, 0, 68);
...
ERR1:
  printf ("%s%s\n", "Error opening file with list of executable", "modules");
  goto END;

ERR2:
  printf ("%s\n", "Empty file with list of modules");
  goto END;

ERR3:
  printf ("%s: %s\n" ,
   "Error when opening file" , SPISOK [I] );
  goto END;

ERR4:
  printf ("%s\n" ,
   "Executable list overflow" );
  goto END;

ERR5:
  printf ("%s\n" ,
   "Object card buffer overflow");
  goto END;
