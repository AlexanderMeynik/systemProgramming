...
int P_CH(void)
{
    ...
    sm = ( int ) ( ADDR -I );                       /*смещения                */
    ARG =
            OBLZ[BAS_IND + CUR_IND + sm ] *  0x100 +//для сравнения нам нужно взять 2 байта из памяти(halfword)
            OBLZ[BAS_IND + CUR_IND + sm + 1];//а после сместить их так, чтобы можно было сравнивать с одержимым регистра
...
int P_STH()
{
    ...
    for (i=0; i<2; i++)                             //т.к. мы записываем в правые 2 байта регистра, берём только их.
...
int P_LH()
{
    ...
    sm = (int) ( ADDR - I );
    VR[R1] =
            OBLZ[BAS_IND + CUR_IND + sm  ] *  0x100 +//загрузим содержимое памяти в 2 правых байта регистра
...
int P_AH()
{
    ...
    ADDR = VR[B] + VR[X] + D;                       /*вычисление абс.адреса и */
    sm = ( int ) ( ADDR -I );                       /*смещения                */
    ARG = OBLZ[BAS_IND + CUR_IND + sm ] *  0x100 +    //для корректного сложения надо получить
          OBLZ[BAS_IND + CUR_IND + sm + 1];      //для аргумента аналогичное представление
...
int P_SH()
{
    ...
    ADDR = VR[B] + VR[X] + D;                       /*вычисление рабочего     */
    sm = ( int ) ( ADDR - I );                      /*адреса и смещения       */
    ARG = OBLZ[BAS_IND + CUR_IND + sm ] *  0x100 +     //аналогично, нам нужно получить нужное представление
          OBLZ[BAS_IND + CUR_IND + sm + 1];
    VR[R1] = VR[R1] - ARG;                          /*выч-ие из 1-го операнда */
    return 0;                                       /*успешное заверш.прогр.  */
}
